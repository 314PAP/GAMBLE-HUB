export const GAME_INFOS = {
  ...
  6: {
    title: "HI-LOW",
    html: '<p>Karetní hra, ve které odhadujete hodnotu další karty. Hraje se s kartami s hodnotami <strong>od 1 do 10</strong>.</p><ul><li><strong>Začátek hry:</strong> Na začátku je vygenerována počáteční karta v rozmezí 2 až 9.</li><li><strong>Jak hrát:</strong> Tipněte si, zda bude další karta <strong>VYŠŠÍ ▲</strong> nebo <strong>NIŽŠÍ ▼</strong> než ta aktuální.</li><li><strong>Při shodě (stejná karta):</strong> Pokud má nová karta stejnou hodnotu, není možné uhodnout a sázka se prohrává.</li><li><strong>Výhra:</strong> Výplata se dynamicky počítá podle počtu výherných karet: více možných výher = nižší násobek, méně možností = vyšší násobek.</li></ul><div class="mt-4 pt-3 border-t border-[rgba(255,255,255,0.06)]"><button class="w-full py-2 px-3 text-xs font-bold text-[var(--neon-cyan)] border border-[rgba(255,255,255,0.1)] rounded-xl bg-[rgba(255,255,255,0.03)] hover:bg-[rgba(255,255,255,0.06)] transition-colors cursor-pointer" onclick="otevriAbbrevModal()">📊 Číselné zkratky</button></div>',
  },
};
