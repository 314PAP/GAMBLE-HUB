...
export class AccountsManager {
  ...
  render(onSelect, onDelete) {
    ...
    usernames.forEach((username) => {
      ...
      selectBtn.innerHTML = `<span class="truncate flex-1 min-w-0 text-[clamp(0.7rem,1.8vw,0.95rem)]">${wrapEmoji(escapeHtml(username))}</span><span class="shrink-0 whitespace-nowrap text-neon-green text-glow-green text-[clamp(0.55rem,1.2vw,0.7rem)]"><span class="score-display">${formatLargeNumber(balance)}${COIN_SVG}</span></span>`;
