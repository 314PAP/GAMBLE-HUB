export class AccountsManager {
  ...
  render(onSelect, onDelete) {
    ...
    usernames.forEach(username => {
      ...
      selectBtn.innerHTML = `<span class="truncate">${username}</span><span class="ml-auto truncate" style="color: var(--neon-green); text-shadow: 0 0 5px var(--neon-green-glow);">${balance}&nbsp;<svg class="coin-icon-svg" viewBox="0 0 24 24" fill="none" stroke="var(--neon-gold)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:16px;height:16px;vertical-align:middle;"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg></span>`;
